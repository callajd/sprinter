...
///
/// - The **execution channel** — it vends a fresh live ``ExecutionEvent`` feed on each
///   `executionEvents` call (modelling BE1's per-(re)start feed), keeping the latest
///   continuation so a test can `emit`/`finish`/`fail` the current feed, so the
///   reused ``ExecutionViewModel``'s transcript builds live.
...
final class PlannerFakeBackend: Backend {
  ...
  init(
    knownExecution: ExecutionId, materializeResult: Result<WorkstreamId, ContractError>,
    gated: Bool = false
  ) {
  ...
  // MARK: - Unused board surface (the planner uses only execution + createWorkstreamFromPlan)
