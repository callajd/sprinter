...
struct ExecutionModelTests {
  @Test("decodes every ExecutionEvent variant and round-trips it")
  func decodesExecutionEvents() throws {
    let events = try Golden.decode([ExecutionEvent].self, from: "execution-events")
  ...
  @Test("maps the turn-lifecycle ExecutionEvent variants")
  func mapsLifecycleEvents() throws {
    ...
    #expect(events[12] == .executionIdle)
  ...
  @Test("maps tool and signal ExecutionEvent variants")
  ...
  @Test("decodes the dual-modality executionEvents-stream envelope")
  func decodesOffsetExecutionEvents() throws {
    let items = try Golden.decode([OffsetExecutionEvent].self, from: "offset-execution-events")
    ...
    // existing ExecutionEvent fold consumes, regardless of whether an offset was present.
    ...
    // per-execution coordinate. The durable offsets are monotonic — the value a client feeds
  ...
  @Test("decodes every ExecutionInput mode (images optional)")
  func decodesExecutionInputs() throws {
    let inputs = try Golden.decode([ExecutionInput].self, from: "execution-inputs")
