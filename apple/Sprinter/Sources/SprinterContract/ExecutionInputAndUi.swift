/// The neutral execution I/O types — ``Usage``, ``ExecutionInput``, ``UiAnswer`` and
/// ``UiResponse`` — plus the execution enums, mirrored from `@sprinter/domain`'s
/// execution model. Each `String`-raw enum's raw value is the exact wire token.
...
/// Severity of an ``ExecutionEvent`` notice / ``TranscriptEntry`` notice entry.
...
/// How an ``ExecutionInput`` is driven into an execution.
public enum ExecutionInputMode: String, Codable, CaseIterable, Sendable {
  case prompt
  case steer
  case followUp
}
...
/// Input driven INTO an execution. `images` is an optional-key field; `mode`
/// distinguishes a fresh prompt, a mid-turn steer, or a follow-up.
public struct ExecutionInput: Codable, Equatable, Sendable {
  public let text: String
  public let images: [String]?
  public let mode: ExecutionInputMode

  public init(text: String, images: [String]?, mode: ExecutionInputMode) {
    self.text = text
    self.images = images
    self.mode = mode
  }
