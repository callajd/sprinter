/// The neutral, maximally reactive execution-event stream type — mirror of the
/// contract's `ExecutionEvent` tagged union (D17 / INV-REACTIVE). It carries turn
...
/// `encodeIfPresent`.
public enum ExecutionEvent: Codable, Equatable, Sendable {
  case turnStarted
...
extension ExecutionEvent {
  public init(from decoder: any Decoder) throws {
    ...
    guard
      ...
      throw DecodingError.dataCorruptedError(
        forKey: .tag,
        in: container,
        debugDescription: "Unknown ExecutionEvent tag: \(tag)"
      )
    }
    self = event
  }
  ...
  private static func decodeLifecycle(
    ...
    case "ExecutionIdle": return .executionIdle
...
extension ExecutionEvent {
  public func encode(to encoder: any Encoder) throws {
    ...
    switch self {
    case .turnStarted, .turnCompleted, .executionIdle, .contextCompacted:
  ...
  private func encodeLifecycle(into container: inout KeyedEncodingContainer<CodingKeys>) throws {
    ...
    case .executionIdle: try container.encode("ExecutionIdle", forKey: .tag)
...
/// A single streamed `executionEvents` item: an ``ExecutionEvent`` paired with an OPTIONAL DURABLE
/// per-execution `offset`. This ONE channel serves BOTH execution modalities — live
...
/// A SETTLED execution's durable transcript replays and the stream completes rather than the old
/// `ExecutionNotFound`. Consumers that only need the event unwrap ``event``.
public struct OffsetExecutionEvent: Codable, Equatable, Sendable {
  public let offset: Int?
  public let event: ExecutionEvent

  public init(offset: Int? = nil, event: ExecutionEvent) {
    self.offset = offset
    self.event = event
  }
}
