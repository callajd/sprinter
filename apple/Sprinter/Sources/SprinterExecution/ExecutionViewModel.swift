...
/// The one reusable **interactive-execution view model** (BE3.1 / D9): the
/// `@Observable @MainActor` surface an execution view renders, driving ANY execution id.
///
/// It is built ON BE1's ``InteractiveExecution`` — it does NOT reimplement the
/// execution channel. Over the injected ``Backend`` port (INV-PORT) it exposes:
///
...
///   ``InteractiveExecution/events``, so the view re-renders as events arrive.
...
///   mirrored ``ContractError/executionNotFound(id:)``.
...
public final class ExecutionViewModel {
  /// The execution this view model drives.
  public let executionId: ExecutionId

  private let execution: InteractiveExecution
  ...
  /// cache; ``InteractiveExecution/events`` is what drives the view.
  ...
  public init(backend: any Backend, executionId: ExecutionId) {
    ...
    self.execution = InteractiveExecution(backend: backend, executionId: executionId)
  }

  /// The live feed projected into the view-facing transcript. Reading
  /// ``InteractiveExecution/events`` establishes observation transitively (so the view
  ...
  public var lifecycle: ExecutionLifecycle {
    if execution.isRunning { return .live }
    if execution.terminationError != nil { return .dropped }
  ...
  /// Subscribes to the live execution feed. **Idempotent while already running** (a
  ...
  /// Drives input into the execution (a fresh prompt, a mid-turn steer, or a
  ...
  public func send(_ input: ExecutionInput) async throws {
    try await execution.send(input)
  }

  /// Interrupts the in-flight turn (D9 — every execution is interruptible); resolves
  /// on success or throws the mirrored ``ContractError/executionNotFound(id:)``.
  ...
  public func answer(requestId: String, _ answer: UiAnswer) async throws {
    try await execution.answer(requestId: requestId, answer)
  }
}
