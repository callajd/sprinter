...
/// Projects an execution's ``ExecutionEvent`` feed into the view-facing ``Transcript``
/// (BE3.1 / D17) — the pure, `Sendable`, offline-testable transform the execution
/// view model renders.
///
...
/// ``InteractiveExecution/outstandingRequests``, not the transcript, so it is folded
/// here as a no-op.
///
...
public enum TranscriptProjection {
  /// Rebuilds the transcript from an execution's event feed.
  public static func project(_ events: [ExecutionEvent]) -> Transcript {
    var builder = Builder()
    for event in events {
  ...
  /// tolerates — because ``project(_:)`` is a deterministic left-fold and the execution
  ...
  public final class Memo {
    ...
    private var foldedTail: ExecutionEvent?

    public init() {}

    /// The transcript for `events`, continuing the fold from the last call. `events`
    /// must be an append-only growth of the prior call's feed (the execution feed's
    ...
    public func project(_ events: [ExecutionEvent]) -> Transcript {
      // Fast path: unchanged feed. An equal count means an unchanged feed ONLY for the
      // append-only execution feed; the tail check confirms it and fails SAFE (falls
  ...
  private struct Builder {
    ...
    mutating func ingest(_ event: ExecutionEvent) {
      switch event {
      case .turnStarted, .turnCompleted, .executionIdle:
    ...
    private mutating func ingestLifecycle(_ event: ExecutionEvent) {
      switch event {
      case .turnStarted:
        isTurnActive = true
    ...
    private mutating func ingestMessage(_ event: ExecutionEvent) {
      switch event {
    ...
    private mutating func ingestTool(_ event: ExecutionEvent) {
      switch event {
    ...
    private mutating func ingestSignal(_ event: ExecutionEvent) {
      switch event {
      case .uiRequestRaised:
        // Surfaced inline via `InteractiveExecution.outstandingRequests`, not the
        // transcript.
        break
