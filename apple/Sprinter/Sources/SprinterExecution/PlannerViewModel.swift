...
/// execution's product is either turned into a new workstream or rejected.
///
...
/// execution whose product materializes into the work graph.
///
/// It is `@Observable @MainActor` and it does NOT re-solve the interactive-execution
/// surface — it **reuses** BE3.1's ``ExecutionViewModel`` (the live transcript,
/// `send`/`interrupt`, the inline `extension_ui_request` round-trip, and the
/// idempotent single-consumer feed lifecycle) for a FRESH planning execution. The
...
public final class PlannerViewModel {
  /// The reused interactive-execution surface (BE3.1). The view renders planning as a
  /// normal execution through this: ``ExecutionViewModel/transcript``,
  /// ``ExecutionViewModel/send(_:)`` / ``ExecutionViewModel/interrupt()``,
  /// ``ExecutionViewModel/outstandingRequests`` + ``ExecutionViewModel/answer(requestId:_:)``,
  /// and the ``ExecutionViewModel/lifecycle``.
  public let execution: ExecutionViewModel
  ...
  /// only ``outcome`` (and the reused ``execution``) drives the view.
  @ObservationIgnored private let backend: any Backend

  public init(backend: any Backend, planningExecutionId: ExecutionId) {
    self.backend = backend
    self.execution = ExecutionViewModel(backend: backend, executionId: planningExecutionId)
  }
  ...
  /// Materializes the planning execution's product into the work graph — the
  /// distinct, explicit step off the execution.
  ///
