...
/// A message as the execution view renders it (BE3.1): a user prompt or an
...
/// A tool invocation as the execution view renders it: the call (name + input)
...
/// One rendered item in the execution transcript. Its ``id`` is the reconciliation
...
/// The view-facing projection of an execution's live feed: the ordered transcript
...
public struct Transcript: Equatable, Sendable {
  ...
  /// The empty transcript — an execution with no events yet.
  public static let empty = Transcript(items: [], isTurnActive: false, lastUsage: nil)
}

/// The terminal-distinguishable lifecycle of an execution view (D9 / the carried BE2
/// single-consumer constraint): whether the feed has started, is live, ended
/// cleanly, or dropped abnormally.
public enum ExecutionLifecycle: Equatable, Sendable {
  /// The feed has never been started.
  case idle
  /// The live feed is currently subscribed.
  case live
  /// The feed finished cleanly (the execution ended) or was intentionally stopped.
  case ended
  /// The feed terminated abnormally — a transport drop or a failure `Exit`
  /// (`InteractiveExecution.terminationError` carries the cause).
  case dropped
}
