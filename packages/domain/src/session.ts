/**
 * The owned, provider-neutral session model (D16 / D17).
 *
 * These are OUR types with plain names — `SessionEvent` (never
 * `AgentSessionEvent`), `SessionInput`, `UiResponse`. They must not leak any Pi
 * concept: nothing here imports or names a `Rpc*` type or a Pi-specific field,
 * and there is zero dependency on any `@earendil-works/pi-*` package. Our shape
 * MIRRORS Pi's already-general abstraction (Pi generalizes across model
 * providers) but we own it as our Effect `Schema`; the FE2.2/Track A adapter
 * translates Pi's `AgentSessionEvent` → our `SessionEvent` at the boundary
 * (INV-NAMING, INV-PORT).
 *
 * {@link SessionEvent} is maximally reactive (D17 / INV-REACTIVE): the union
 ...
 * The session model is agnostic to a tool's payload shape, so it stays
...
/** Severity of a {@link SessionEvent} `Notice`. */
...
/**
 * The neutral session-event stream type — transcript-grade and maximally
 ...
 * `SessionEvent`, not `AgentSessionEvent` (INV-NAMING).
 */
...
/**
 * Input driven INTO a session. `mode` distinguishes a fresh `prompt`, a `steer`
