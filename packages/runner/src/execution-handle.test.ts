/**
 * `ExecutionHandle` + boundary-translation coverage (AE1.2).
 *
 * Two layers of test:
 *   - PURE translation/encoding: `translateServerEvent` over representative Pi
 *     wire events → expected owned `ExecutionEvent`, and `encodeInput` /
...
import type { ExecutionEvent } from "@sprinter/domain";
...
const expectTranslation = (event: PiServerEvent, expected: ReadonlyArray<ExecutionEvent>): void => {
  expect(translateServerEvent(event)).toEqual(expected);
};

// ============================================================================
// Translation — turn & execution lifecycle
// ============================================================================
...
it("translates agent settle and context compaction", () => {
  expectTranslation({ type: "agent_settled" }, [{ _tag: "ExecutionIdle" }]);
  expectTranslation(
...
it("translates message execution entries into durable EntryAppended records", () => {
...
it("drops non-message execution entries", () => {
  expectTranslation(
    {
      type: "entry_appended",
      entry: {
...
it("translates execution-info and thinking-level changes to StatusChanged", () => {
...
it("encodes each ExecutionInput mode into its Pi command", () => {
...
it.effect("fans out translated ExecutionEvents to a subscriber, in order", () =>
  Effect.gen(function* () {
...
it.effect("drives an ExecutionInput into the execution as the matching Pi command", () =>
  Effect.gen(function* () {
    const fake = yield* makeFakePi;
    yield* Effect.scoped(
      Effect.gen(function* () {
