 ...
 *
 * Task AE1.2 adds the neutral {@link ExecutionHandle} ({@link ./execution-handle.ts}):
 * it consumes the substrate and translates Pi's wire events into the owned
 * `ExecutionEvent` model, so everything exported here is expressed ONLY in neutral
 * types (`ExecutionEvent` / `ExecutionInput` / `UiResponse` from `@sprinter/domain`).
 * No Pi wire type crosses this boundary (INV-BOUNDARY).
 *
 * The public surface: the execution factory + handle, its terminal result, the
...
export { make as makeExecution, ExecutionResult } from "./execution-handle.ts";
export type { ExecutionHandle } from "./execution-handle.ts";
