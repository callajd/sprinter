/**
 * `ExecutionHandle` — the owned, provider-neutral surface of one live Pi execution
 ...
 * types, and this module translates them into our neutral execution model so
 ...
 * `ExecutionEvent` / `ExecutionInput` / `UiResponse` from `@sprinter/domain`.
 *
 ...
 * `ExecutionEvent` to clients (see {@link make}).
 */
...
/**
 * The neutral terminal outcome of an execution, resolved by {@link ExecutionHandle.result}.
 ...
 * wire value).
 */
export const ExecutionResult = Schema.TaggedUnion({
  Completed: {},
...
/**
 * One live execution, expressed entirely in owned neutral types. Obtained from
...
export interface ExecutionHandle {
  ...
  /**
   * The live stream of owned {@link ExecutionEvent}s — maximally reactive (D17):
   ...
   */
  readonly events: Stream.Stream<ExecutionEvent, PiTransportError>;
  /** Drive an {@link ExecutionInput} (prompt / steer / followUp) into the execution. */
  readonly send: (input: ExecutionInput) => Effect.Effect<void, PiRpcError | PiTransportError>;
  ...
  /** Resolve the execution's terminal outcome (awaits completion). */
  readonly result: Effect.Effect<ExecutionResult>;
}

// ============================================================================
// Translation: Pi server events → owned ExecutionEvent (INV-BOUNDARY)
// ============================================================================
...
/**
 * Translate a durable Pi execution entry into the neutral `EntryAppended`. Only
 ...
 */
const translateEntry = (entry: PiSessionEntry): ReadonlyArray<ExecutionEvent> => {
  ...
  switch (message.role) {
    ...
    case "assistant": {
      ...
      const toolCalls = message.content.reduce<ReadonlyArray<ExecutionEvent>>(
        (acc, block) =>
          block.type === "toolCall"
            ? [
                ...acc,
...
const translateUiRequest = (event: PiRpcExtensionUIRequest): ReadonlyArray<ExecutionEvent> => {
  switch (event.method) {
    case "select":
      return [
        {
 ...
 * {@link ExecutionEvent}s. TOTAL and exhaustive via discriminated matching on the
 ...
 */
export const translateServerEvent = (event: PiServerEvent): ReadonlyArray<ExecutionEvent> => {
  switch (event.type) {
...
/**
 * Encode a neutral {@link ExecutionInput} into the matching Pi command. The three
 * neutral modes map to Pi's `prompt` / `steer` / `follow_up`; the transport
 * assigns the correlation `id`, so none is set here.
 */
export const encodeInput = (input: ExecutionInput): PiRpcCommand => {
...
/**
 * The fan-out buffer capacity for an execution's event stream. Generous: overflow
...
/**
 * Spawn a `pi --mode rpc` execution and expose it as a neutral {@link ExecutionHandle}.
 ...
 * resolves {@link ExecutionHandle.result}.
 */
export const make = (
  config?: PiProcessConfig,
): Effect.Effect<ExecutionHandle, PlatformError, ChildProcessSpawner.ChildProcessSpawner | Scope> =>
  Effect.gen(function* () {
    const transport = yield* makeTransport(config);
    const pubsub = yield* PubSub.sliding<Take.Take<ExecutionEvent, PiTransportError>>({
      capacity: EVENTS_CAPACITY,
      replay: EVENTS_CAPACITY,
    });
    const result = yield* Deferred.make<ExecutionResult>();
    ...
    const send = (input: ExecutionInput): Effect.Effect<void, PiRpcError | PiTransportError> =>
